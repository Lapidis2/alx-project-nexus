...
const TopProduct: React.FC = () => {
  ...
  // Memoizing the total revenue sum to avoid recalculation on each render
  const totalRevenueSum = useMemo(
    () => topProducts.reduce((acc, product) => acc + product.totalRevenue, 0),
    [topProducts]
  );
  ...
  return (
    <section className="flex flex-col border-2 w-full rounded-md p-4">
